...
MODULE Class_DSSFileType
  USE GeneralUtilities    , ONLY: UpperCase                                  , &
                                  IntTotext
  USE TimeSeriesUtilities , ONLY: TimeStampLength                            , &
                                  TimeStampToJulian                          , &
                                  TimeStampToYear4000                        , &
                                  DSSStyleDate                               , &
...
! ******************************************************************
  ...
  ! --- PARAMETERS
  ! -------------------------------------------------------------
  INTEGER,PARAMETER          :: DefaultPathNameLength        = 80      , &
                                DefaultPartNameLength        = 32      , &
                                DefaultDataUnitAndTypeLength = 8
  CHARACTER(LEN=3),PARAMETER :: DefaultFileNameExtension     = 'DSS'
  ...
  TYPE,EXTENDS(BaseFileType) :: DssInFileType   !BaseFileType component is not used, all that info will be stored in the DSSFileListNode data
      ...
      CHARACTER(LEN=TimeStampLength)                   :: DateOfLastDataRead   = ''            !Date of the last data that was read from the DSS file
      INTEGER                                          :: Interval_InMinutes   = 0             !Time interval of data in minutes
      CHARACTER(LEN=DefaultPathNameLength),ALLOCATABLE :: PathNames(:)                         !Path names used      
  ...
  TYPE,EXTENDS(BaseFileType) :: DssOutFileType   !BaseFileType component is not used, all that info will be stored in the DSSFileListNode data
      ...
      CHARACTER(LEN=TimeStampLength)                   :: DataStartDateAndTime  = ''          !Starting date and time for the first value in the data storage
...
! ******************************************************************
    ...
    IF (.NOT. FileIsOpen) THEN
      ...
      IF (.NOT. ASSOCIATED(DSSFileListHead)) THEN  !Initial entry
          ...
          IF (ErrorCode .NE. 0) THEN
              IF (PRESENT(FileOpenCode)) THEN
                  ...
                  RETURN
              ELSE
                  CALL SetLastMessage('Error in allocating memory for DSS file list for file '//TRIM(ADJUSTL(FileName))//'!',iFatal,ThisProcedure)
                  iStat = -1
                  RETURN
              END IF
          END IF
      ...
      IF (ErrorCode.NE.0) THEN
          IF (PRESENT(FileOpenCode)) THEN
              ...
              RETURN
          ELSE
              CALL SetLastMessage('Error in opening file '//TRIM(ADJUSTL(FileName))//'!',iFatal,ThisProcedure)
              iStat = -1
              RETURN
          END IF
      END IF
  ...
  SUBROUTINE New_DssInFile(ThisFile,FileName,lInputFile,AccessType,FileOpenCode,iStat)
    ...
    IF (.NOT.lExist) THEN
        CALL SetLastMessage('DSS file '//TRIM(FileName)//' does not exist for data retrieval!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
...
! ******************************************************************
  ...
  SUBROUTINE SetParameters_DSSInFile(ThisFile,PathNames,NColumnsOfData,NRowsOfData,iStat)
    CLASS(DSSInFileType)                            :: ThisFile
    CHARACTER(LEN=DefaultPathNameLength),INTENT(IN) :: PathNames(:)
    ...
    CHARACTER(LEN=DefaultPartNameLength)   :: APart,BPart,CPart,DPart,EPart,FPart
    ...
    IF (nrows*NColumnsOfData.NE.SIZE(PathNames)) THEN
        CALL SetLastMessage('Error in sizing of path name array for '//ThisFile%Name//'!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    ALLOCATE (ThisFile%PathNames(nrows*NColumnsOfData)          , &
              ...
              STAT=ErrorCode                                    )
    IF (ErrorCode .NE. 0) THEN
        CALL setLastMessage('Error in allocating memory for the path names for '//ThisFile%Name//'!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    ...
    DO indx=1,SIZE(PathNames)
      CALL ZUFPN(APart ,DummyInt                    , &
                 ...
                 ErrorCode                          )
      IF (ErrorCode .NE. 0) THEN
          CALL SetLastMessage('Error in finding the interval length of a pathname in file '//ThisFile%Name//'!',iFatal,ThisProcedure)
          iStat = -1
          RETURN
      END IF
      ...
      SELECT CASE (indx)
        ...
        CASE DEFAULT
          IF (Interval_InMinutes .NE. ThisFile%Interval_InMinutes) THEN
              CALL SetLastMessage('The data intervals for records in DSS input file '//ThisFile%Name//' should be the same!',iFatal,ThisProcedure) 
              iStat = -1
              RETURN
          END IF
  ...
  SUBROUTINE SetParameters_DSSOutFile_PathNames(ThisFile,PathNames,DataUnit,DataType,SimulationStartTime,NTimeSteps,NColumnsOfData,NRowsOfData,iStat)
    ...
    IF (ErrorCode .NE. 0) THEN
        CALL SetLastMessage('Error in allocating memory for the path names for file '//ThisFile%Name//'!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    ThisFile%PathNames = PathNames
    ...
    ALLOCATE (ThisFile%DataUnit(SIZE(DataUnit)) , &
              ...
              STAT=ErrorCode                    )
    IF (ErrorCode .NE. 0) THEN
        CALL SetLastMessage('Error in allocating memory for the data unit and type for '//ThisFile%Name//'!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
  ...
  SUBROUTINE SetParameters_DSSOutFile_Parts(ThisFile,APart,BPart,CPart,EPart,FPart,DataUnit,DataType,SimulationStartTime,NTimeSteps,NColumnsOfData,NRowsOfData,iStat)
    ...
    CHARACTER(LEN=DefaultPartNameLength)   :: PartNames(6)
    ...
    ALLOCATE (ThisFile%DataUnit(SIZE(DataUnit)) , &
              ...
              STAT=ErrorCode                    )
    IF (ErrorCode .NE. 0) THEN
        CALL SetLastMessage('Error in allocating memory for the data unit and type for file '//ThisFile%Name//'!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
...
! ******************************************************************
  ...
  SUBROUTINE ReadRegularIntervalScalar_DssInFile(ThisFile,SimulationTime,Data,Stat,iStat)
    ...
    SELECT TYPE (Data)
        ...
        CLASS DEFAULT
            CALL SetLastMessage('The specified data type cannot be read from file '//ThisFile%Name//'!',iFatal,ThisProcedure) 
            iStat = -1
  ...
  CONTAINS
    ...
    SUBROUTINE ReadData(rData)
      ...
      !Local variables
      CHARACTER(LEN=TimeStampLength) :: WorkTimeStamp
  ...
  SUBROUTINE ReadTimeSeries_ForTimeRange_DSSInFile(ThisFile,iPathNameIndex,cBeginDateAndTime,cEndDateAndTime,nActualOutput,Data,rDataDates,Stat,iStat)     
    ...
    CHARACTER                              :: cCurrentDateAndTime*TimeStampLength,BeginDate*20,BeginTime*4, &
                                              cPathName*80,DataUnit*8,DataType*8
    CHARACTER(LEN=32)                      :: PathNameParts(6)
...
! ******************************************************************
  ...
  SUBROUTINE WriteRegularIntervalArray_DssOutFile(ThisFile,SimulationTime,Data,FinalPrint)
    ...
    SELECT TYPE (Data)
        ...
        CLASS DEFAULT
            CALL LogMessage('Array of data type acnnot be written to file '//ThisFile%Name//'!',iWarn,ThisProcedure)
  ...
  SUBROUTINE WriteRegularIntervalMatrix_DssOutFile(ThisFile,SimulationTime,Data,FinalPrint)
    ...
    CHARACTER(LEN=DefaultPathNameLength)        :: PathName
    CHARACTER                                   :: StartDate*20,StartTime*4
    CHARACTER(LEN=DefaultDataUnitAndTypeLength) :: DataUnit,DataType
...
! ******************************************************************
  ...
  SUBROUTINE GetDSSFile_ByIndex(DSSFileIndex,DSSFile,iStat)
    ...
    DO
        ...
        IF (.NOT. ASSOCIATED(DSSFile%Next)) THEN
            CALL SetLastMessage('File with index '//TRIM(IntToText(DSSFileIndex))//' cannot be located in the list of opened DSS files!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
  ...
  SUBROUTINE GetDSSFile_ByName(FileName,DSSFile,iStat)
    ...
    DO
        ...
        IF (.NOT. ASSOCIATED(DSSFile%Next)) THEN
            CALL SetLastMessage('File '//TRIM(FileName)//' cannot be located in the list of opened DSS files!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        DSSFile => DSSFile%Next
  ...
  SUBROUTINE CheckLength(String,iStat)
    ...
    IF (LEN_TRIM(ADJUSTL(String)) .GT. DefaultDataUnitAndTypeLength) THEN
      CALL SetLastMessage('DataUnit or DataType is longer than allowed!',iFatal,ThisProcedure)
      iStat = -1
    END IF
  ...
  SUBROUTINE DeleteRecords(OldFile,PathName,StartDate,StartTime,NValues,iStat) 
    ...
    CHARACTER(LEN=DefaultPathNameLength)   :: RecordToDelete
    ...
    DO indx=1,SIZE(PathName)
      ...
      CALL ZUFPN(APart ,DummyInt          , &
                 ...
                 ErrorCode                )
      IF (ErrorCode .NE. 0) THEN
          CALL SetLastMessage('Error in finding the interval length of the pathname '//TRIM(PathName(indx))//'!',iFatal,ThisProcedure)
          iStat = -1
          RETURN
      END IF
      ErrorCode = 1
    ...
    IF (ErrorCode .NE. 0) THEN
        CALL SetLastMessage('Error in opening temporary file to squeeze '//TRIM(OldFile%Name),iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    ...
    IF (ErrorCode .NE. 0) THEN
        CALL SetLastMessage('Error in opening file '//TRIM(OldFile%pFileData%Name)//' after squeezing!',iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
  ...
  FUNCTION FormPathName(PartNames) RESULT(CPATH)
    ...
    CHARACTER(LEN=DefaultPathNameLength) :: CPATH

    !Local variables
    INTEGER                              :: LengthOfCPath
    CHARACTER(LEN=DefaultPartNameLength) :: LocalPartNames(6)
