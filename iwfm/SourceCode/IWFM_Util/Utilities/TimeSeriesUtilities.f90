...
MODULE TimeSeriesUtilities
  ...
  INTEGER,PARAMETER :: TimeStampLength=16
  ...
  TYPE TimeStepType
    ...
    CHARACTER(LEN=TimeStampLength) :: CurrentDateAndTime = ''
    CHARACTER(LEN=TimeStampLength) :: EndDateAndTime     = ''    
    REAL(8)                        :: DeltaT             = 0.0
  ...
  ! --- PARAMETERS
  ! -------------------------------------------------------------
  INTEGER,PARAMETER :: RecognizedIntervals_InMinutes(20) = [1,2,3,4,5,10,15,20,30,60,120,180,240,360,480,720,1440,10080,43200,525600]
  CHARACTER(LEN=6),PARAMETER :: RecognizedIntervals(20) = ['1MIN  ' , &
                                                           '2MIN  ' , &
                                                           '3MIN  ' , &
                                                           '4MIN  ' , &
                                                           '5MIN  ' , &
...
!////////////////////////////////////////////////////////////////////////////////
  ...
  FUNCTION IsTimeIntervalValid(cInterval) RESULT(iIndex)
    ...
    DO iIndex=1,SIZE(RecognizedIntervals)
        IF (TRIM(UpperCase(ADJUSTL(cInterval))) .EQ. TRIM(RecognizedIntervals(iIndex))) EXIT
    END DO
    
    IF (iIndex .GT. SIZE(RecognizedIntervals)) iIndex = 0
  ...
  SUBROUTINE DayMonthYearToJulianDate(iDay,iMonth,iYear,iJulian,iStat)
    ...
    IF (iYear .LT. 1) THEN
      IF (PRESENT(iStat)) THEN
        ...
        RETURN
      ELSE
        CALL LogMessage('Cannot convert dates with calendar year less than 1 to Julian day!',iFatal,ThisProcedure)
      END IF
    END IF
    ...
    IF (iMonth.LT.1  .OR.  iMonth.GT.12) THEN
      IF (PRESENT(iStat)) THEN
        ...
        RETURN
      ELSE
        CALL LogMessage('Incorrect number for month ('//TRIM(IntToText(iMonth))//')!',iFatal,ThisProcedure)
      END IF
    END IF
    ...
    IF (ErrorCode .NE. 0) THEN
      IF (PRESENT(iStat)) THEN
        ...
        RETURN
      ELSE
        CALL LogMessage('Day ('//TRIM(IntToText(iDay))//') of the month is incorrect given the month ('//TRIM(IntToText(iMonth))//')!',iFatal,ThisProcedure)
      END IF
    END IF
  ...
  FUNCTION LeapYearCorrection(TimeStampIn) RESULT(TimeStampOut)
    CHARACTER(LEN=TimeStampLength),INTENT(IN)::TimeStampIn
    CHARACTER(LEN=TimeStampLength)::TimeStampOut
    ...
    IF (iYear .EQ. 4000) THEN
      ...
      CALL LogMessage(MessageArray(1:2),iFatal,ThisProcedure)
    END IF
  ...
  FUNCTION IsTimeStampValid(ALine) RESULT (TimeStampValidity)
    ...
    !Local variables
    CHARACTER(LEN=1),PARAMETER::Slash='/',Colon=':',Underscore='_'
    CHARACTER(LEN=TimeStampLength)::TimeStamp
    ...
    IF (CountOccurance(Slash,TimeStamp).NE.2) RETURN

    !Check if possible time stamp includes one of "_"
    IF (CountOccurance(Underscore,TimeStamp).NE.1) RETURN

    !Check if possible time stamp includes one of ":"
    IF (CountOccurance(Colon,TimeStamp).LT.1) RETURN
    
    !Check time stamp has the right number of characaters
    IF (LEN_TRIM(ADJUSTL(TimeStamp)) .NE. TimeStampLength) RETURN
  ...
  FUNCTION StripTimeStamp(ALine,Location) RESULT(TimeStamp)
    ...
    !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    LocationOfFirstSlash=FirstLocation(Slash,ALine)
    ...
    TimeStampEnd   = TimeStampBegin + TimeStampLength - 1  ;  TimeStampEnd   = MAX(TimeStampEnd , TimeStampBegin)
  ...
  SUBROUTINE TimeStampToJulianDateAndMinutes(TimeStamp,JulianDate,MinutesAfterMidnight,STAT)
    CHARACTER(LEN=TimeStampLength),INTENT(IN) :: TimeStamp
  ...
  FUNCTION DSSStyleDate_FromTimeStamp(TimeStamp) RESULT(Date)
    ...
    Date=TRIM(IntToText(Day))//' '//TRIM(Months(Month))//' '//TRIM(IntToText(Year))
  ...
  SUBROUTINE GetJulianDatesBetweenTimeStampsWithTimeIncrement(Interval_InMinutes,cBeginDateAndTime,cEndDateAndTime,rJulianDates)
    ...
    CHARACTER :: cTimeStamp*TimeStampLength
  ...
  SUBROUTINE AdjustTimeStampWithYear4000(AdjustedTimeStamp,TimeStamp,Year4000Flag)
    CHARACTER(LEN=TimeStampLength)::AdjustedTimeStamp
    CHARACTER(LEN=TimeStampLength),INTENT(IN)::TimeStamp
  ...
  FUNCTION TimeStampToYear4000(TimeStamp) RESULT(ConvertedTimeStamp)
    CHARACTER(LEN=TimeStampLength),INTENT(IN)::TimeStamp
    CHARACTER(LEN=TimeStampLength)::ConvertedTimeStamp
  ...
  FUNCTION IncrementTimeStamp(TimeStamp,Interval_InMinutes,NumberOfIntervals) RESULT(IncrementedTimeStamp)
    ...
    CHARACTER(LEN=TimeStampLength) :: IncrementedTimeStamp
...
!////////////////////////////////////////////////////////////////////////////////
  ...
  SUBROUTINE SetTSDCacheSize(FileName            , &
    ...
    IF (ErrorCode.NE.0) CALL LogMessage('Error in allocating data storage array for file '//TRIM(FileName),iFatal,ThisProcedure) 
        
    !Allocate memory for the time storage array
    IF (PRESENT(TimeArray)) THEN
      ALLOCATE (TimeArray(nbatch),STAT=ErrorCode)
      IF (ErrorCode.NE.0) CALL LogMessage('Error in allocating time storage array for file '//TRIM(FileName),iFatal,ThisProcedure) 
    END IF
  ...
  SUBROUTINE AdjustRateTypeData(r,RateTypeDataArray,ConversionFactor,DataInterval,LastDataDate)
    ...
    CHARACTER(LEN=TimeStampLength),OPTIONAL,INTENT(IN)::LastDataDate ! the date of last data read are supplied as character

    !Local variables
    INTEGER::nrow,ncol
    REAL(8)::r1d(SIZE(r,DIM=1)*SIZE(r,DIM=2))
    REAL(8)::Factor
    CHARACTER(LEN=TimeStampLength)::TimeStampBegin
...
!////////////////////////////////////////////////////////////////////////////////
  ...
  FUNCTION CheckForLessThan(TimeStamp1,TimeStamp2) RESULT(TestResult)
    CHARACTER(LEN=TimeStampLength),INTENT(IN)::TimeStamp1,TimeStamp2
    LOGICAL::TestResult
  ...
  FUNCTION NPeriodsBetweenTimeStamps(DELTAT_InMinutes,BeginTimeStamp,EndTimeStamp) RESULT(NPeriod)
    ...
    CHARACTER :: TempTimeStamp*TimeStampLength
...
! *************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE CTimeStep_To_RTimeStep(UNITT,DELTAT,DELTAT_InMinutes,Status)
    ...
    INTEGER                      :: TimeStepIndex,LocalStatus
    ...
    IF (TimeStepIndex .EQ. 0) THEN
      LocalStatus = -1
      IF (.NOT. PRESENT(Status)) CALL LogMessage(TRIM(UNITT)// ' is not a recognized time step',iFatal,ThisProcedure) 
    END IF

    !Convert time step to Julian date increment "in terms of minutes"
    IF (TimeStepIndex .NE. 0) THEN
        DELTAT_InMinutes = RecognizedIntervals_InMinutes(TimeStepIndex)
    ELSE
        DELTAT_InMInutes = 0
    END IF
    
    !DELTAT is always 1.0 since the unit includes the time step length, e.g. DELTAT = 1 12HOUR
    DELTAT = 1.0

    IF (PRESENT(Status)) Status=LocalStatus
  ...
  FUNCTION TimeIntervalConversion(ToInterval,FromInterval) RESULT(ConversionFactor)
    ...
    IF (ErrorCode.NE.0) CALL LogMessage(TRIM(UpperCase(FromInterval))//' is not a valid time interval!',iFatal,ThisProcedure)

    !Convert ToInterval to minutes
    CALL CTimeStep_To_RTimeStep(UpperCase(ToInterval),DummyReal,ToInterval_InMinutes,ErrorCode)
    IF (ErrorCode.NE.0) CALL LogMessage(TRIM(UpperCase(ToInterval))//' is not a valid time interval!',iFatal,ThisProcedure)
