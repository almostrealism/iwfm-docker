...
! ******************************************************************
  ...
  CHARACTER(LEN=300)          :: MessageArray(50)
...
! *************************************************************
  ...
  SUBROUTINE GetLastMessage(cMessage)
    ...
    iLenLastMessage = LEN(cLastMessage)
    
    IF (iLenMessage .LT. iLenLastMessage) THEN    
        cMessage = cLastMessage(1:iLenMessage)
    ELSE
        cMessage(1:iLenLastMessage) = cLastMessage
    END IF
  ...
  SUBROUTINE PrintRunTime()
    ...
    IF (WarningsGenerated) THEN
      ...
      IF (ALLOCATED(ThisLogFile))  &
        MessageArray(3)=TRIM(MessageArray(3))// &
                        'FOR DETAILS CHECK FILE '''//TRIM(ThisLogFile%Name)//'''.'//LineFeed
