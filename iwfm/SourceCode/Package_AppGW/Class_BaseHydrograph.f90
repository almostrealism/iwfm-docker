...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE HydOutput_New(IsForInquiry,InFile,cWorkingDirectory,AppGrid,Stratigraphy,iHydFor,UNITLTOU,CPart,TimeStep,HydOutput,iStat)
    ...
    CALL HydrographList_New(AppGrid,Stratigraphy,iHydFor,NHyd,FactXY,InFile,HydOutput%Hyd_AtNode,HydOutput%Hyd_AtXY,HydOutput%OrderedHydList,iStat)
    ...
    IF (IsForInquiry) THEN
        ...
        CALL PrepHydInFile_ForInquiry(cAbsPathFileName,iHydFor,CPart,HydOutput%Hyd_AtNode,HydOutput%Hyd_AtXY,HydOutput%OrderedHydList,TimeStep,HydOutput%InFile_ForInquiry,iStat)
    ELSE
        ALLOCATE (HydOutput%OutFile)
        CALL PrepHydOutFile(cAbsPathFileName,iHydFor,UNITLTOU,CPart,HydOutput%Hyd_AtNode,HydOutput%Hyd_AtXY,HydOutput%OrderedHydList,TimeStep,HydOutput%OutFile,iStat)
    END IF
    
  END SUBROUTINE HydOutput_New
  
  
  ! -------------------------------------------------------------
  ! --- NEW LIST OF HYDROGRAPH DATA
  ! -------------------------------------------------------------
  SUBROUTINE HydrographList_New(AppGrid,Stratigraphy,iHydFor,NHyd,FactXY,InFile,Hyd_AtNode,Hyd_AtXY,OrderedHydList,iStat)
    TYPE(AppGridType),INTENT(IN)                 :: AppGrid
    TYPE(StratigraphyType),INTENT(IN)            :: Stratigraphy
    INTEGER,INTENT(IN)                           :: iHydFor,NHyd
    ...
    CHARACTER                    :: ALine*1000,cErrorMsg*300,cHydDescriptor*11,cHydDescriptorCapital*11
    ...
    IF (ErrorCode .NE. 0) THEN
        CALL SetLastMessage('Error in allocating memory for ordered '//TRIM(cHydDescriptor)//' hydrograph list!'//NEW_LINE('x')//TRIM(cErrorMsg),iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    ...
    DO indx=1,NHyd
        ...
        DO indx1=1,3
            READ (ALine,*) DummyArray(indx1)
            iLoc = FirstLocation(' ',ALine)
            IF (iLoc .EQ. 0) THEN
                CALL SetLastMessage('Error in data entry for '//TRIM(cHydDescriptor)//' hydrograph specification '//TRIM(IntToText(indx))//'!',iFatal,ThisProcedure)
                iStat = -1
                RETURN
            END IF
        ...
        IF (ID .NE. indx) THEN
            MessageArray(1) = TRIM(cHydDescriptorCapital)//' hydrograph print-out specifications must be entered sequentially.'
        ...
        IF (.NOT.(iHydType .EQ. iHyd_AtXY   .OR.   iHydType .EQ. iHyd_AtNode)) THEN
            CALL SetLastMessage(TRIM(cHydDescriptorCapital)//' hydrograph type listed for hydrograph ID '//TRIM(IntToText(ID))//' is not recognized!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        ...
        IF (iHydLayer .LT. 0   .OR.   iHydLayer .GT. Stratigraphy%NLayers) THEN
            CALL SetLastMessage(TRIM(cHydDescriptorCapital)//' hydrograph layer listed for hydrograph ID '//TRIM(IntToText(ID))//' is outside model bounds!',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
        
        !Based on hydrograph type, read the rest of the entries
        IF (iHydType .EQ. iHyd_AtXY) THEN
            DO indx1=1,2
                READ (ALine,*) DummyArray(indx1)
                iLoc = FirstLocation(' ',ALine)
                IF (iLoc .EQ. 0) THEN
        ...
        SELECT CASE (iHydType)
            CASE (iHyd_AtXY)
                ...
                IF (iElem .EQ. 0) THEN
                    CALL SetLastMessage(TRIM(cHydDescriptorCapital)//' hydrograph ID '//TRIM(IntToText(ID))//' is not in the model domain!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
                IF (iHydLayer .NE. 0) THEN
                    IF (ALL(Stratigraphy%ActiveNode(Nodes,iHydLayer) .EQ. .FALSE.))  &
                        CALL LogMessage(TRIM(cHydDescriptorCapital)//' hydrograph ID '//TRIM(IntToText(ID))//' is located in an inactive layer!',iInfo,ThisProcedure)
                ELSE
                ...
                IF (iStat .EQ. -1) RETURN
            CASE (iHyd_AtNode)
                IF (iHydNode .LT. 1   .OR.   iHydNode .GT. AppGrid%NNodes) THEN
                    CALL SetLastMessage('The node number for '//TRIM(cHydDescriptor)//' hydrograph ID '//TRIM(IntToText(ID))//' is less than 1 or greater than the number of nodes modeled!',iFatal,ThisProcedure)
                    iStat = -1
                    RETURN
                END IF
                IF (iHydLayer .NE. 0) THEN
                    IF (.NOT. Stratigraphy%ActiveNode(iHydNode,iHydLayer))  &
                        CALL LogMessage('Groundwater node '//TRIM(IntToText(iHydNode))//' at layer '//TRIM(IntToText(iHydLayer))//' for '//TRIM(cHydDescriptor)//' hydrograph ID '//TRIM(IntToText(ID))//' is inactive!',iInfo,ThisProcedure)
                ELSE
                    IF (ALL(Stratigraphy%ActiveNode(iHydNode,:) .EQ. .FALSE.))  &
                        CALL LogMessage('There are no active layers at node '//TRIM(IntToText(iHydNode))//' for '//TRIM(cHydDescriptor)//' hydrograph ID '//TRIM(IntToText(ID))//'!',iInfo,ThisProcedure)
                END IF
                X                 = AppGrid%Node(iHydNode)%X
                Y                 = AppGrid%Node(iHydNode)%Y
                CALL AppGrid%FEInterpolate(X,Y,iElem,Nodes,Coeff)   
                aNodeHyd%cName    = ALine
                aNodeHyd%iLayer   = iHydLayer
                aNodeHyd%iElement = iElem
                aNodeHyd%iNode    = iHydNode
    ...
    IF (ErrorCode .NE. 0) THEN
        CALL SetLastMessage('Error in allocating memory for '//TRIM(cHydDescriptor)//' hydrograph data!'//NEW_LINE('x')//TRIM(cErrorMsg),iFatal,ThisProcedure)
        iStat = -1
        RETURN
    END IF
    CALL HydList%Reset()
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE HydOutput_Kill(HydOutput)
    TYPE(HydOutputType) :: HydOutput
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  FUNCTION HydOutput_IsDefined(HydOutput) RESULT(lDefined)
    TYPE(HydOutputType),INTENT(IN) :: HydOutput
    LOGICAL                        :: lDefined
    
    lDefined = .FALSE.
    IF (ALLOCATED(HydOutput%OutFile)) THEN
        IF (HydOutput%OutFile%iGetFIleType() .NE. UNKNOWN) lDefined = .TRUE.
    ELSEIF (ALLOCATED(HydOutput%InFile_ForInquiry)) THEN
        IF (HydOutput%InFile_ForInquiry%File%iGetFileType() .NE. UNKNOWN) lDefined = .TRUE.
    END IF
    
  END FUNCTION HydOutput_IsDefined
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE GetHydrographCoordinates(HydOutput,GridNodeCoordinates,X,Y)
    CLASS(HydOutputType),INTENT(IN) :: HydOutput
    ...
    DO indxHyd=1,SIZE(HydOutput%OrderedHydList)
        SELECT CASE (HydOutput%OrderedHydList(indxHyd)%iHydType)
            CASE (iHyd_AtXY)
                ...
                X(indxHyd) = HydOutput%Hyd_AtXY(indx)%X
                Y(indxHyd) = HydOutput%Hyd_AtXY(indx)%Y
                
            CASE (iHyd_AtNode)
                indx       = HydOutput%OrderedHydList(indxHyd)%indx
                iNode      = HydOutput%Hyd_AtNode(indx)%iNode
                X(indxHyd) = GridNodeCoordinates(iNode)%X
                Y(indxHyd) = GridNodeCoordinates(iNode)%Y
...
! ******************************************************************
  ...
  SUBROUTINE ReadHydrograph_AtLocation(HydOutput,iHydID,cOutputBeginDateAndTime,cOutputEndDateAndTime,rOutputFactor,rConversionFactor,nActualOutput,rOutputDates,rOutputValues,iStat)
    ...
    !Local variables
    INTEGER :: FileReadCode,nHydTotal
    ...
    !Read data
    CALL HydOutput%InFile_ForInquiry%ReadData(iHydID,cOutputBeginDateAndTime,cOutputEndDateAndTime,nActualOutput,rOutputValues,rOutputDates,FileReadCode,iStat)  
    IF (iStat .EQ. -1) RETURN
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE HydOutput_PrintResults(Stratigraphy,iHydFor,rValues,rFactor,TimeStep,lEndOfSimulation,HydOutput)
    TYPE(StratigraphyType),INTENT(IN) :: Stratigraphy
    INTEGER,INTENT(IN)                :: iHydFor
  ...
  END SUBROUTINE HydOutput_PrintResults
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE PrepHydOutFile(cFileName,iHydFor,UNITLTOU,CPartIn,Hyd_AtNode,Hyd_AtXY,OrderedHydList,TimeStep,OutFile,iStat)
    ...
    INTEGER                      :: NHyd,indx,indx1,Layers(SIZE(OrderedHydList)),                 &
    ...
    IF (OutFile%iGetFileType() .EQ. DSS) THEN
        IF (.NOT. TimeStep%TrackTime) THEN
            CALL SetLastMessage('DSS files for '//TRIM(cHydDescriptor)//' hydrograph printing can only be used for time-tracking simulations.',iFatal,ThisProcedure)
            iStat = -1
            RETURN
        END IF
    END IF
    ...
    DO indx=1,NHyd
      ...
      SELECT CASE (OrderedHydList(indx)%iHydType)
          CASE (iHyd_AtNode)
              ...
              Elements(indx) = Hyd_AtNode(indx1)%iElement
              GWNodes(indx)  = Hyd_AtNode(indx1)%iNode
          CASE (iHyd_AtXY)
              Layers(indx)   = Hyd_AtXY(indx1)%iLayer 
              Elements(indx) = Hyd_AtXY(indx1)%iElement
              GWNodes(indx)  = 0
      END SELECT
    END DO
  ...
  ! -------------------------------------------------------------
  SUBROUTINE PrepHydInFile_ForInquiry(cFileName,iHydFor,CPart,Hyd_AtNode,Hyd_AtXY,OrderedHydList,TimeStep,InFile,iStat)
    ...
    IF (iGetFileType_FromName(cFileName) .EQ. DSS) THEN
        !Form pathnames
        DO indx=1,NHydrographs
            BPart            = TRIM(IntToText(indx)) // ':L' // TRIM(IntToText(Layers(indx))) // ':E' // TRIM(IntToText(Elements(indx))) // ':GW' // TRIM(IntToText(GWNodes(indx)))
