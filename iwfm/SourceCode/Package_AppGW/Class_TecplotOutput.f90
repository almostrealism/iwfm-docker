...
! ******************************************************************
  ...
  SUBROUTINE PrintInitialValues(TPOutput,AppGrid,StrmConnectivity,rIniValues,Factors,cFormat,cVarNames,TimeStep)
    ...
    CHARACTER :: Header(3)*3000,Text*3000,cZoneName*TimeStampLength
    ...
    DO indxNode=1,NNodes
      WRITE (Text,cFormat) AppGrid%Node(indxNode)%X,AppGrid%Node(indxNode)%Y,(rIniValues(indxNode,indxVar)*Factors(indxVar),indxVar=1,iDimVar)
    ...
    DO indxElem=1,NElements
      ...
      Nodes(1:NVertex) = AppGrid%Element(indxElem)%Vertex(1:NVertex)
  ...
  SUBROUTINE PrintResults(TPOutput,rValues,rFactor,TimeStep)
    ...
    !Local variables
    CHARACTER :: Text*3000,cZoneName*TimeStampLength
    INTEGER   :: indx,indx1
...
! ******************************************************************
  ...
  FUNCTION ZoneName(TimeStep) RESULT(cName)
    TYPE(TimeStepType),INTENT(IN)  :: TimeStep
    CHARACTER(LEN=TimeStampLength) :: cName
