...
! ******************************************************************
  ...
  PUBLIC :: NPondedCrops                                 ,  &
            ...
            PondedAgLandUse_ComputeETFromGW_Max
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE PondedAgLandUse_New(IsForInquiry,cFileName,cWorkingDirectory,FactCN,AppGrid,TimeStep,NTimeSteps,cIWFMVersion,PondLand,iStat)
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE PondedAgLandUse_ReadTSData(TimeStep,AppGrid,ETData,PondLand,iStat)
    TYPE(TimeStepType),INTENT(IN) :: TimeStep
    TYPE(AppGridType),INTENT(IN)  :: AppGrid
    ...
    IF (ETData%IsUpdated()) THEN
        ...
        DO indxElem=1,AppGrid%NElements
            ETc     = ETData%GetValues(PondLand%Crops(:,indxElem)%iColETc)
            iRegion = AppGrid%AppElement(indxElem)%Subregion
            DO indxCrop=1,NPondedCrops
                PondLand%RegionETPot(indxCrop,iRegion) = PondLand%RegionETPot(indxCrop,iRegion) + ETc(indxCrop) * PondLand%Crops(indxCrop,indxElem)%Area
            END DO
        END DO
    END IF
...
! ******************************************************************
  ...
  SUBROUTINE WriteLWUseFlowsToBudRawFile(AppGrid,RLUArea,RPump,RDeli,RUpstrmElemRunoff,PondedAg)
    ...
    DO indxCrop=1,NBudgetCrops
      ...
      ElemValue                                  = PondedAg%Crops(iBudgetCrops(indxCrop),:)%ETFromGW_Actual
  ...
  SUBROUTINE WriteRootZoneFlowsToBudRawFile(AppGrid,RLUArea,RPump,RDeli,RUpstrmElemRunoff,ElemPrecip,ElemGenericMoist,Area,PondedAg)
   ...
   REAL(8),INTENT(IN)              :: Area(AppGrid%NElements,PondedAg%NBudgetCrops)
    ...
    DO indxCrop=1,NBudgetCrops
      ...
      ElemValue                                       = PondedAg%Crops(iCrop,:)%ETFromGW_Actual 
...
! ******************************************************************
  ...
  SUBROUTINE PondedAgLandUse_Simulate(AppGrid,ETData,DeltaT,Precip,GenericMoisture,SoilsData,HydCondPonded,ElemSupply,ElemsToGW,SolverData,lLakeElem,PondedAg,iStat)
    ...
    TYPE(PondedAgDatabaseType),TARGET      :: PondedAg
    INTEGER,INTENT(OUT)                    :: iStat
    ...
    REAL(8)                      :: AchievedConv,Area,ETc(NPondedCrops),HydCond,TotalPorosity,              &
                                    ...
                                    Infilt(3),SoilM_Array(3),ETPartition(3),ETc_effect
    ...
    ASSOCIATE (pCrops          => PondedAg%Crops                       , &
      ...
      DO indxElem=1,AppGrid%NElements
        ...
        DO indxCrop=1,NPondedCrops
            pCrop => pCrops(indxCrop,indxElem)
            ...
            !ET from GW
            pCrop%ETFromGW_Actual = MIN(ETc(indxCrop) , pCrop%ETFromGW_Max)
            ETc_effect            = ETc(indxCrop) - pCrop%ETFromGW_Actual
      ...
      DO indxElem=1,AppGrid%NElements
        ...
        DO indxCrop=1,NPondedCrops
          ASSOCIATE (pCrop => pCrops(indxCrop,indxElem))
            ...
            FieldCapacityCrop = FieldCapacity * RootDepth
            GM                = GMElem * RootDepth
            ETc_effect        = ETc(indxCrop) - MIN(ETc(indxCrop) , pCrop%ETFromGW_Max)
...
! ******************************************************************
  ...
  ! -------------------------------------------------------------
  SUBROUTINE PondedAgLandUse_ComputeETFromGW_Max(DepthToGW,Sy,CapillaryRise,PondedAgLand)
