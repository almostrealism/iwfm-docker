...
C   ***
 5    IF(.NOT.LFIRST) GO TO 10                                          Hu
C5    IF(.NOT.LFIRST)           GO TO 11                                ML
C     CALL PBLOCK                                                       H
C   ***
C   ***
...
C     IF (CNAME(1:6).EQ.'CONIN$') LKEYB = .TRUE.                        ML
      IF (CNAME(1:5).EQ.'STDIN')  LKEYB = .TRUE.                        uL
      IF (CNAME(1:5).EQ.'/dev/')  LKEYB = .TRUE.                        u
      IF (CNAME(1:5).EQ.'stdin')  LKEYB = .TRUE.
...
C        IF ( CLBUFM(IMACP+1)(1:1) .NE. CSPL(1:1)                       ML
C    +          .AND.       IMACP .NE. IMXMCP ) GO TO 10                ML
      ENDIF
      ENDFILE ISCRT
      REWIND ISCRT
      ENDIF
