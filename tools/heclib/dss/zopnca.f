...
C
      IF (LOPNCA) THEN
         CLOSE (UNIT=ICUNIT)
         IF (ILKHND.GT.-1) CALL CLOSF(ILKHND, ISTAT)
         LOPEN = .FALSE.
      ENDIF
C
      IF (LOPNCD) THEN
...
C    *      IOSTAT=IERR, ACTION='READ')
            INQUIRE (UNIT=IUNIT, OPENED=LOPEN)
         ENDIF
C
         IF ((IERR.NE.0).OR.(.NOT.LOPEN)) THEN
            IF (MLEVEL.GE.1) WRITE (MUNIT,30) CATFIL(1:ILAST),
     *       CCOND(1:NCOND), IERR
 30         FORMAT (/,' **** ZOPNCA - ERROR:  Unable to Access the',
     *       '  Catalog File ****',/,' Catalog File: ',A,',  Condensed',
     *       '  Catalog: ',A,/,' Error: ',I6)
            ...
            IF (IUNIT.EQ.ICUNIT) THEN
               CALL OPENF(CATFIL(1:ILAST-1)//'k', 10, ILKHND, ISTAT)
