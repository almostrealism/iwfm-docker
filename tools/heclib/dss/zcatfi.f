...
C     IF (LCATST) CALL CHRWT (MUNIT, CHAR(13)//CHAR(10)//'  0% Complete'Md
C    * // ',      0 Records'// CHAR(13), 32)                            Md
      IF (LCATST) CALL CHRWT (MUNIT, '  0% Complete'                    u
     * // ',      0 Records'// CHAR(13), 30)                            u
C
C
...
C     Should the status (% complete) be printed
 ...
 120  FORMAT (I3,'% Complete;',I7,' Records')
      CALL CHRWT (MUNIT, CPATH(1:29)//CHAR(13), 30)                     u
      ENDIF
      ENDIF
C
...
C     Should we show a pseudo regular interval pathname?
      IF (LPSEUDO.AND.(IPNBIN(JPNBIN+NBPWPA+KBPINTL).GT.0)) THEN
         CALL ZPseudoRTS(CPATH(1:NPATH), CKPATH,
...
C
 ...
 850  FORMAT (' ----- DSS --- ZCATFI:  Catalog interrupted.')
      GO TO 800
C
 900  CONTINUE
