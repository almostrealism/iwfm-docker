      SUBROUTINE ZCOLIST (IFLTAB, IFPOS, CPATH, NPATH, ISTAT)
C
...
C
 ...
 20   FORMAT (T6,'-----DSS---Debug:  Enter ZCOLIST;  Unit:',I5,/,
     * T6,'Seed path = ',A)
C
C
...
C
      CALL ZCOLPATH(CPATH, NPATH, CCPATH, LCOLL)
...
C        the address of the first bin
         ...
         MHASH = IFLTAB(KHASH)
         CALL ZHASH(CCPATH, NPATH, MHASH, JHASH)       
         JPBIN = 1	
...
C           Should not get here
            WRITE(MUNIT,*)' IFLTAB CORRUPT IN ZCOLIST, KTABLE ',
     *                    IFLTAB(KTABLE)
            CALL ZABORT(IFLTAB, 100, 'ZCOLIST', 0, IFLTAB(KTABLE), ' ')
            RETURN
        ENDIF         
      ELSE
...
C         If it is way off, then it is a file error
      ...
      ENDIF
      CALL ZGTREC (IFLTAB, IPBIN, IFLTAB(KBNSIZ), IPBADD, .FALSE.)
...
C
 ...
 840     FORMAT (T6,'-----DSS---Debug:  Exit  ZCOLIST, Status:', I3)
         IF (ISTAT.EQ.0) THEN
