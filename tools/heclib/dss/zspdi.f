...
C
      ...
      LOGICAL LABEL, LFOUND, LDOUBLE, LWRITE
      INTEGER MAXLABEL
C
      INCLUDE 'zdssmz.h'
C
...
C        Store only a single coordinate array (one curve) and label
         ...
         IF (LDOUBLE) THEN
            ...
            CALL ZPTREC(IFLTAB, DVALUES, N, IADD, .FALSE.)
            IF (IFLTAB(KDSWAP).NE.0) CALL ZDSWAP(DVALUES, N)
         ELSE
            IADD = INFO(NPPWRD+KIADAT) + (NCURVE * NORD)
            CALL ZPTREC(IFLTAB, SVALUES, NORD, IADD, .FALSE.)
         ENDIF
         NVALS = NORD
...
C       Now store the label
        IF (LABEL) THEN
            ...
            CALL ZGTREC(IFLTAB, IGBUFF, NIHEAD, IADD, .FALSE.)
...
C              NWDS is the number of words for each extended label
	         ...
	         IF ((NIHEAD.GE.N).AND.(NWDS.LT.26)) THEN
	          ...
	          ENDIF
             ENDIF
          CALL ZPTREC(IFLTAB, IGBUFF, NIHEAD, IADD, .FALSE.)
        ENDIF
...
C        Now store the ordinates
         ...
         IF (LDOUBLE) THEN
            N = NORD  *2
            IF (IFLTAB(KDSWAP).NE.0) CALL ZDSWAP(DVALUES, N)
            CALL ZPTREC(IFLTAB,DVALUES, N, INFO(NPPWRD+KIADAT),.FALSE.)
            IF (IFLTAB(KDSWAP).NE.0) CALL ZDSWAP(DVALUES, N)
         ELSE
            CALL ZPTREC(IFLTAB, SVALUES, NORD, INFO(NPPWRD+KIADAT),
     *                   .FALSE.)
         ENDIF
...
C
 ...
 901  FORMAT (/,' *** ERROR:  zspd;  Error writting record',
     * /,' Pathname: ',A,/)
      GO TO 800
C
 905  CONTINUE
      IF (MLEVEL.GE.1) WRITE (MUNIT, 906) NORD, CPATH(1:NPATH)
 906  FORMAT (/,' *** ERROR:  zspd;  The Number of Ordinates is Less',
...
C
 ...
 931  FORMAT (/,' *** ERROR:  zspd;  Exceeded size limit for number',
     * ' of data ***',/,' Pathname: ',A)
