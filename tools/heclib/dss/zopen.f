...
C        an advisory mode, unless requested to force multi-user mode
      	 IF (LMULTU) THEN
            ...
            LMULTU = .FALSE.
         ELSE
            CALL ISONLOCALDRIVE(CNAME, LLOCAL)
            IF (LLOCAL) THEN
               IFLTAB(KMULT) = 3
            ELSE
               IFLTAB(KMULT) = 2
...
C     CALL isonlocaldrive (CNAME(1:ILEN+1), ILOCAL)                     M
C     IF (ILOCAL.EQ.0) IFLTAB(KREMOTE) = 1                              M
...
C     Get the best estimate of the file size.
      CNAME(ILEN+1:ILEN+1) = CHAR(0)                                    u
      CALL FILESIZEN (CNAME(1:ILEN+1), ISIZE, JSTAT)                    u
      CNAME(ILEN+1:ILEN+1) = ' '                                        u
C     CALL filesize64 (IFLTAB(KHANDL), ISIZE8)                          Md
      IFSIZE8 = ((ISIZE8 - 1)/ 4) + 1
      IF (IFSIZE8.LT.100) IFSIZE8 = 9999999
      IFLTAB(KFSIZE) = I8TOI4(IFSIZE8)
      ENDIF
C
      ENDIF
C
