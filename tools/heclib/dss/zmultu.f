...
C           at the same time!
            DO 32 I=1,200                                               u
               CALL WAITS (0.1)                                         u
               CALL lockdss (IFLTAB(KHANDL), 3, IBYTE, IWORD(2), JERR)  u
               IF (JERR.EQ.0) GO TO 35                                  u
 32         CONTINUE                                                    u
C           CALL lockdss (IFLTAB(KHANDL), 1, IBYTE, IWORD(2), JERR)     Md
C           CALL lockdss (IFLTAB(KHANDL), 0, IBYTE, IWORD(2), KERR)     Md
C           IF (JERR.EQ.0) GO TO 35                                     Md
C           We do not have access to lock this file!
            WRITE ( MUNIT, 60) IUNIT
            WRITE (MUNIT, 34) JERR, KERR
 34         FORMAT(' Lock error code: ', I5,'  Unlock error: ', I5)
            CALL ZABORT (IFLTAB, 210, 'ZMULTU', IUNIT, IFLTAB(KLOCK),
     *       'Can not lock file')
            RETURN
         ENDIF
...
C     On unix, we need to be compatiable with DSS versions prior to 6-K
      call oldLockDSS (IFLTAB(KHANDL), 1,IFLTAB(KLOCKB),IWORD(2), JERR) u
      IFLTAB(KLOCK) = 2
...
C
C
C     IF ((IFLTAB(KLOCK).EQ.2).AND.(IFLTAB(KMULT).LE.2)) THEN           Md
C     Compatability for unix DSS versions prior to 6-K
      IF ((IFLTAB(KLOCK).EQ.2).AND.(IFLTAB(KMULT).LE.3)) THEN           u
C
...
C
C     On unix, we need to be compatiable with DSS versions prior to 6-K
      call oldLockDSS (IFLTAB(KHANDL), 0,IFLTAB(KLOCKB),IWORD(2), JERR) u
C
...
C
      RETURN
      END
      subroutine oldLockDSS (ihandle, mode, position, size, istat)      u
c
      integer ihandle, mode, position, size, istat                      u
      integer oldPosition                                               u
c
c     Compatability for versions of dss prior to 6-K
c
      irec = position / 512                                             u
      ibyte = position - (irec * 512)                                   u
      oldPosition = (irec * 127) + 1 + ibyte                            u
c
      call lockdss (ihandle, mode, oldPosition, size, istat)            u
c
      return                                                            u
      end                                                               u
