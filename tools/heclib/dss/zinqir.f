...
C     This occured on Unix in 2001 where ResSim wrote doubles to Unix
C     for rating tables.  The doubles were not binarily compatiable with
C     the PC and resulted in stange numbers that caused invalid rating
C     operations, (and often program aborts).  The issue was resolved
C     soon after, and it had been thought that all DSS files had been
C     updated, but some from backup kept returning.
...
C     NUMB returns 0 if no issues, or the numbe or records that had the
C     pdd endian problem.
C     NUMB  < 0 if error
      ELSE IF (CFLAG.EQ.'CPDD') THEN
         ...
         ELSE
            INSTR = 4
         ENDIF
         CALL ZCKPDD (IFLTAB, INSTR, NFIXED, ISTAT)
         IF (ISTAT.LE.0) THEN
            NUMB = ISTAT
         ELSE
            NUMB = NFIXED
