      SUBROUTINE ZRITSI (IFLTAB, CPATHNAME, JULS, ISTIME, JULE, IETIME,
...
C     interval data set, but the user sees the general interval      
      ...
      IF (I.GT.2) THEN
        CALL ZPseudoRTS(CPATHNAME(1:NPATH), CPATH, INTLPS, 1, ISTAT)
        IF (ISTAT.EQ.0) THEN
...
C
      IF (ILPART(4).GT.3) THEN
         IF (CPATH(IBPART(4):IBPART(4)+2).EQ.'TS-') THEN
            KB = KLBUFF
            CALL ZREADX (IFLTAB, CPATH, IIHEAD, KIHEAD, NNIHEAD,
...
C
      KB = KLBUFF
      CALL ZREADX (IFLTAB, CPATH1, IIHEAD, KIHEAD, NNIHEAD,
     * IDUM, 0, N, IUHEAD, KUHEAD, NUHEAD, ILBUFF, KB, NB, 2, LF)
      CALL ZINQIR (IFLTAB, 'STATUS', CSCRAT, JSTAT)
      IF (JSTAT.NE.0) GO TO 940
C
