...
250    continue
    ...
    read_model_sample_file: do
      ...
      if(right_word(1)-left_word(1).gt.19) then                                !CFB
        call num2char(iline,aline)
...
!    being able to create ins file)
	...
	real                 :: elim
        ...
        character(len=20),allocatable, dimension(:,:)  :: hdiffpairs   !MJT, !CFB
        hdiffunit = 189                                                !MJT
...
!   format.
    ...
    integer, parameter :: MAXHYD   = 10000
    integer, parameter :: MAXTIME  =  1500
    integer, parameter :: iitimeopt =     8
    
    real               :: rtime(MAXTIME)
    real               :: rval(INUMHYD,MAXHYD,MAXTIME)
    character*20       :: hydid(INUMHYD,MAXHYD)                              !CFB
    character*1000     :: adate

    integer i,j,k,ifail,ls(6),rs(6),iline,ntime,iyr,imon,iday,ihydtype
...
!                  (rval(ihydtype,j,ntime),j=1,nout(ihydtype))
                ...
                if(ihydtype.eq.SUBSID) then          ! subsidence
                    read(iiwfmfile(ihydtype),'(A22,10000F12.2)',end=800) junk,&
                      (rval(ihydtype,j,ntime),j=1,nout(ihydtype))
                else if(ihydtype.eq.TILEDR) then     ! tile drain
                    read(iiwfmfile(ihydtype),'(A22,10000F12.2)',end=800) junk,&
                      (rval(ihydtype,j,ntime),j=1,nout(ihydtype))
                else if(ihydtype.eq.STREAM) then     ! stream 
                    read(iiwfmfile(ihydtype),'(A22,10000F14.2)',end=800) junk,&
                      (rval(ihydtype,j,ntime),j=1,nout(ihydtype))
                else  !(ihydtype.eq.GWHEAD)          ! groundwater
                    read(iiwfmfile(ihydtype),'(A22,10000F12.4)',end=800) junk,&
                      (rval(ihydtype,j,ntime),j=1,nout(ihydtype))
                end if
            end do
800         ntime=ntime-1
