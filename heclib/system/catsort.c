...
void fortwrite(char *message, int32_t munit, int32_t advance) {
	fortranwrite_(&munit, message, &advance, strlen(message));
}
...
void catsort(
	...
	while(atoi(line) == 0) {
		...
		else   header_lines = (char **)malloc(header_line_count * sizeof(char*));
	...
	for (group_count = 1;; ++group_count) {
		...
		line_data = (char *)malloc(max_lines * LINE_LEN * sizeof(char));
	...
	*current_numb = -3;
	...
	if (tempfile_count > 0) {
		...
		// prime the merge //
		//-----------------//
		done = malloc(tempfile_count * sizeof(int32_t));
...
void catsort_(
	...
	catfilename = (char *)malloc(filename_len + 1);
	if (catfilename == NULL) {
		fortwrite("", *munit, TRUE);
