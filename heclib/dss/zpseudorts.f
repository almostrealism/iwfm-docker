      SUBROUTINE ZPseudoRTS(CFROMPATH, CTOPATH, INTL,IACTION,ISTATUS)
...
C       Look for the tilde to identify a pseudo Reg TS path
        ...
        ENDIF
        ISTATUS = 1
       CALL ZGINTL(INTL,CFROMPATH(IBPART(5)+1:IEPART(5)),NODATA,ISTATUS)
        ...
        CTOPATH = CFROMPATH(1:IBPART(5)-1)//CEPART(1:N)//
...
C
        ISTATUS = 2
        CALL ZGINTL (INTL, CEPART, NODATA, ISTATUS)
        IF (ISTATUS.NE.0) THEN
            RETURN
        ENDIF 
        CALL CHRLNB(CEPART, N) 
