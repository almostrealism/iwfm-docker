...
C    * // CHAR(13), 16)
      IF (LSQSTA) CALL CHRWT (MUNIT, ' 0% Complete' // CHAR(13), 14)    u
C     IF (LSQSTA) CALL CHRWT (MUNIT, '+ 0% Complete' // CHAR(13), 14)   Md
C
...
C     Should the status (% complete) be printed
      ...
      IP = ((IREC-1)*100) / NRECS
      WRITE (CPATH, 25) IP
 25   FORMAT (I3,'% Complete')                                          u
C25   FORMAT ('+',I3,'% Complete')                                      Md
...
C     Check for pseudo regular time series data
      IF ((ITYPE.GE.110).AND.(ITYPE.LE.120)) THEN
        IADD = IIHADD + 4
        CALL ZGTREC (IFTOLD, INTL_PSEUDO, 1, IADD, .FALSE.)
      ENDIF
C
C
...
C
C
      IF (LSQSTA) CALL CHRWT (MUNIT, '100' // CHAR(13) // CHAR(10), 6)  u
C     IF (LSQSTA) CALL CHRWT (MUNIT, '+100' // CHAR(13) // CHAR(10), 6) Md
C
 800  CONTINUE
      LSQSTA = .FALSE.
      LTSCMP = .FALSE.
