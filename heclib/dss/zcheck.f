...
C     irregular interval that behaves like a regular interval
      ...
      IF (I.GT.5) THEN
      CLPATH = ' '
         CALL ZPseudoRTS(CKPATH, CLPATH, I, 1, ISTAT)
         IF (ISTAT.EQ.0) THEN
            CKPATH = CLPATH
...
C     (On a write, ZMULTU will check)
      IF (.NOT.LWRITE) THEN
         CALL ZCKMUL (IFLTAB)
         IF (IFLTAB(KMULT).EQ.2) CALL ZRDPRM (IFLTAB, .TRUE.)
      ENDIF
C
C
...
C     IF ((.NOT.LWRITE).AND.(IFLTAB(KMULT).EQ.2)) THEN                  Md
C        CALL flushf (IFLTAB(KHANDL), IERR)                             Md
C     ENDIF                                                             Md
C
C
      LINTAB = .FALSE.
...
C     have the same hash code, and then we only have to search that one hash
      CALL ZCOLPATH (CPATH, NPATH, CLPATH, LCOLL)
      IF (LCOLL) THEN
         ...
         ELSE
            IF (IFLTAB(KCOLL).EQ.0) THEN
                ...
                ELSE
                    IFLTAB(KCOLL) = 1
                    NADD = 1
                    CALL ZPTREC (IFLTAB, IFLTAB(KPERM), NPERM, 
     *                           NADD, .TRUE.)
