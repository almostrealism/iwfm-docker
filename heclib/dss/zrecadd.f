...
C     Check that IFLTAB is valid (e.g., the DSS file is open)
      IF (IFLTAB(1).NE.6) CALL ZERROR (IFLTAB, 5, 'ZRECADD',
     * 0, IFLTAB, ' ', 0, ' ',0)
C
      CALL ZRDINF (IFLTAB, CPATH, NHEAD, NDATA, ISTAT)
...
C
 900  CONTINUE
      ...
      NP = INFO(KINPAT)
      CALL ZERROR (IFLTAB, 11, 'ZRECADD', 0, NADD, CPATH, NPATH, CTPATH,
     * NP)
      ISTAT = -1
      RETURN
C
