      SUBROUTINE ZLOADCACHE (IFLTAB, ISTAT)
C
...
C
     ...
     *                 INTERRUPT, NERROR, MAXERROR
      ...
      INTEGER(8) ISIZE8, IOFSET8, IPOS8
C
C
      IF (MLEVEL.GE.11) WRITE (MUNIT,20) IFLTAB(KHANDL)
 20   FORMAT (T6,'-----DSS---Debug:  Enter ZLOADCACHE;  Handle:',I5)
C
...
C
 ...
 810  FORMAT (T6,'-----DSS---Debug:  Exit  ZLOADCACHE')
C     
      RETURN
      END
