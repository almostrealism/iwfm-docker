...
C
      ...
      IF (IWORD(4).EQ.0) THEN                                           Hu
      CHR(IBEG:IEND) = STR(1:NCMW)                                      HMu
      ELSE                                                              Hu
      CHR(IBEG:IBEG) = STR(4:4)                                         Hu
      CHR(IBEG+1:IBEG+1) = STR(3:3)                                     Hu
      CHR(IBEG+2:IBEG+2) = STR(2:2)                                     Hu
      CHR(IBEG+3:IBEG+3) = STR(1:1)                                     Hu
      ENDIF                                                             Hu
      IBEG = IEND + 1                                                   HMu
 10   CONTINUE                                                          HMu
C
...
C     IF (IWORD(4).NE.0) THEN                                           M
C     KWORDS = NWORDS * IWORD(7)                                        ML
C     CALL HOLCHR (IHOL, 1, KWORDS, CHR, 1)                             ML
C     ENDIF                                                             M
C
C
