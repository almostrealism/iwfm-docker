...
C     IF (IWORD(4).EQ.0) THEN                                           H
      STR(1:NCMW) = CHR(IBEG:IEND)                                      HMu
C     ELSE                                                              H
C     STR(4:4) = CHR(IBEG:IBEG)                                         H
C     STR(3:3) = CHR(IBEG+1:IBEG+1)                                     H
C     STR(2:2) = CHR(IBEG+2:IBEG+2)                                     H
C     STR(1:1) = CHR(IBEG+3:IBEG+3)                                     H
C     ENDIF                                                             H
      IHOL(I) = INT                                                     HMu
      IBEG = IEND + 1                                                   HMu
 10   CONTINUE                                                          HMu
...
C     For Microsoft, use chrhol if we need to swap bytes!
 20   CONTINUE
      IF (IWORD(4).NE.0) THEN                                           Mu
      KWORDS = NWORDS * IWORD(7)                                        MLu
      CALL CHRHOL (CHR, 1, KWORDS, IHOL, 1)                             MLu
      ENDIF                                                             Mu
C
C
